...
namespace Server
{
    public partial class Server : Form
    {
        ...
        public async void startServer()
        {
            ...
            MessageBox.Show(LocalIP.ToString());

         // MessageBox.Show("Server is available");
            //NetworkStream serverStreamm = client.GetStream();
            //string stre = "server is available";
            //byte[] outStreamm = Encoding.ASCII.GetBytes(stre);
            //serverStreamm.Write(outStream, 0, outStream.Length);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    string str = "server is available";
        ...
        private void btnServerStop_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                NetworkStream serverStream = client.GetStream();
                string str = "server is  not available";
                byte[] outStream = Encoding.ASCII.GetBytes(str);
                serverStream.Write(outStream, 0, outStream.Length);
           //     serverStream.Flush();
                listener.Stop();
